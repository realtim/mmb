...
<script language="JavaScript">
    ...
    // Функция добавления впечатления
    function AddLink() {
...
if ($AllowEdit === 1) {
    ...
    if ($viewmode !== 'Add') {
        ...
        //{
        print('<input type="button" onClick = "javascript: if (confirm(\'Вы уверены, что хотите выслать на адрес ' . trim($UserEmail) . ' новый пароль для ' . trim(
                $UserName
            ) . ' будет создан новый пароль и ? \')) { NewPassword(); }"  name="ChangePasswordButton" value="Создать и выслать новый пароль" tabindex="' . (++$TabIndex) . '">' . "\r\n");
        //}
    }
...
if ($Administrator && $viewmode !== 'Add') {
    print('<tr><td class = "input"  style =  "padding-top: 10px;">' . "\r\n");
    print('<input type="button" style="padding-left: 30px;padding-right: 30px;margin-right:15px;" onClick="javascript: if (confirm(\'Вы уверены, что хотите удалить пользователя: ' . trim(
            $UserName
        ) . '? \')) {HideUser();}" name="HideUserButton" value="Удалить пользователя" tabindex="' . (++$TabIndex) . '">' . "\n");
...
// Выводим список команд, в которых участвовал данный пользователь
...
while ($Row = mysqli_fetch_assoc($Result)) {
    ...
    if ($RaidDeveloper) {
        ...
        if ($TeamPlace > 0 && $LevelPointId == 0) {
            $TeamPlaceResult = "место <b>$TeamPlace</b>";
        }
        $TeamUserOff = "";
        //  сход участника
        if ($LevelPointId) {
            $TeamUserOff = "не явился(-ась) в <b>{$Row['levelpoint_name']}</b>";
        }
        if ($TeamDismiss) {
            $TeamUserOff = "команда не явилась на старт";
        }
...
if ($viewmode !== 'Add' && $AllowEdit == 1) {
    // Выводим список устройств, которые относятся к данному пользователю
...
if ($viewmode !== 'Add' && !empty($UserId)) {
    ...
    print('<br/><input type="button" onClick = "SendMessage();"  name="SendMessageButton" value="Отправить" tabindex = "' . (++$TabIndex) . '">' . "\r\n");
    print('</form>' . "\r\n");
}
// Конец проверки на режим правки и авторизованного пользователя


// 04.07.2014  Блок ссылок на впечатления.
if ($viewmode !== 'Add' && UACanLinkEdit($pUserId, $RaidId, $UserId) === 1) {
    // Выводим список впечатлений, которые относятся к данному пользователю
    ...
    $RaidCondition = '';
    if (CSql::userModerator($UserId, $RaidId) == 1 && $UserId <> $pUserId) {
    ...
    while ($Row = mysqli_fetch_assoc($Result)) {
        ...
        print('<option value="' . $Row['linktype_id'] . '" ' . $linktypeselected . '  onclick = "document.UserLinksForm.NewLinkName.disabled=' . $LinkNameDisabled . ';">' . $Row['linktype_name'] . "</option>\n");
    }
    mysqli_free_result($Result);
    print('</select>' . "\n");
...
if ($viewmode !== 'Add' && CRights::canViewUserInvitations($pUserId, $RaidId, $UserId)) {
    // Выводим список
