...
if (!isset($request["X-Sportiduino-Protocol"])) {
    die("Запрос сделан не из приложения под Android");
}
if ($request["X-Sportiduino-Protocol"] != PROTOCOL_VERSION) {
    die("Некорректная версия протокола обмена, требуется " . PROTOCOL_VERSION);
}
...
if (!isset($request["X-Sportiduino-Auth"])) {
    die("В запросе отсутствуют данные о пользователе");
}
[$email, $password] = explode("|", $request["X-Sportiduino-Auth"]);

if (!isset($email)) {
    die("Отсутствует емейл пользователя");
}

if (!preg_match("/[a-zA-Z0-9.-]+@[a-zA-Z0-9.-]+/", $email)) {
    die("Некорректный емейл пользователя");
}

if (!isset($password)) {
    die("Отсутствует пароль пользователя");
}

if (!preg_match("/[a-z0-9]+/", $password)) {
    die("Некорректный пароль пользователя");
}
...
if (!$row) {
    die("Некорректные логин или пароль");
}
...
if (!$can_access) {
    die("Работать с системой могут только админы/модераторы/волонтеры");
}

// Все в порядке, обрабатываем запрос пользователя
if (!isset($request["X-Sportiduino-Action"])) {
    die("Отсутствует код операции, запрошенной у сервера");
}
...
function SendDistance(PDO $pdo, $raid_id)
{
    ...
    $ro_hours = (int)$row["raid_readonlyhoursbeforestart"];
    $sql = null;
    ...
    $max_order = -1;
    $sql = $pdo->prepare(
        "SELECT levelpoint_order, levelpoint_name, pointtype_id, levelpoint_penalty, UNIX_TIMESTAMP(levelpoint_mindatetime) AS mintime, UNIX_TIMESTAMP(levelpoint_maxdatetime) AS maxtime FROM LevelPoints WHERE distance_id = :distance_id AND levelpoint_hide = 0 ORDER BY levelpoint_order ASC"
    );
    ...
    if (count($result) < 2) {
        die("На дистанции отсутствует финиш");
    }
    foreach ($result as $row) {
        if (isset($points["p" . $row["levelpoint_order"]])) {
        ...
        if ($row["levelpoint_order"] > $max_order) {
            $max_order = $row["levelpoint_order"];
        }
        $row["levelpoint_name"] = trim(strtr($row["levelpoint_name"], "\t\n", "  "));
        $points["p" . $row["levelpoint_order"]] = [
            "name" => $row["levelpoint_name"],
            "type" => (int)$row["pointtype_id"],
            "penalty" => (int)$row["levelpoint_penalty"],
    ...
    for ($n = $last; $n > 0; $n--) {
        ...
        if (!$points[$index]["end"]) {
            $points[$index]["end"] = $prev_end;
        } else {
            $prev_end = $points[$index]["end"];
        }
    }
    ...
    $discounts = [];
    $sql = $pdo->prepare(
        "SELECT levelpointdiscount_value AS value, levelpointdiscount_start AS start, levelpointdiscount_finish AS finish FROM LevelPointDiscounts WHERE distance_id = :distance_id AND levelpointdiscount_hide = 0"
    );
    $sql->bindParam("distance_id", $distance_id, PDO::PARAM_INT);
    $sql->execute();
    $result = $sql->fetchAll(PDO::FETCH_ASSOC);
    foreach ($result as $row) {
        if (!$row["value"]) {
            die("Нулевая амнистия для интервала " . $row["start"] . " - " . $row["finish"]);
        }
        if (!isset($points["p" . $row["start"]])) {
            die("Не существует точка " . $row["start"] . " из интервала амнистии");
        }
        if (!isset($points["p" . $row["finish"]])) {
            die("Не существует точка " . $row["finish"] . " из интервала амнистии");
        }
        $discounts[] = ["value" => (int)$row["value"], "start" => (int)$row["start"], "end" => (int)$row["finish"]];
    }
    $sql = null;
    ...
    foreach ($result as $row) {
        ...
        if (isset($teams["t" . $row["team_num"]])) {
            die("Дублирующийся номер команды " . $row["team_num"]);
        }
        if ($row["team_num"] > $max_number) {
            $max_number = $row["team_num"];
        }
        $row["team_name"] = trim(strtr($row["team_name"], "\t\n", "  "));
        $teams["t" . $row["team_num"]] = ["name" => $row["team_name"], "maps" => (int)$row["team_mapscount"], "members" => 0];
    }
    $sql = null;
    if (!count($teams)) {
    ...
    $members = [];
    $sql = $pdo->prepare(
        "SELECT Users.user_id, Teams.team_num, Users.user_name, Users.user_birthyear, Users.user_phone FROM Teams, TeamUsers, Users WHERE distance_id = :distance_id AND team_hide = 0 AND teamuser_hide = 0 AND user_hide = 0 AND TeamUsers.user_id = Users.user_id AND TeamUsers.team_id = Teams.team_id ORDER BY Users.user_id ASC"
    );
    $sql->bindParam("distance_id", $distance_id, PDO::PARAM_INT);
    $sql->execute();
    $result = $sql->fetchAll(PDO::FETCH_ASSOC);
    foreach ($result as $row) {
        if (isset($members["m" . $row["user_id"]])) {
        ...
        $members["m" . $row["user_id"]] = ["team" => $row["team_num"], "name" => $name, "phone" => $row["user_phone"]];
    }
    $sql = null;
...
function ReceiveResults(PDO $pdo, $user_id)
{
    ...
    if (!isset($_POST["data"])) {
        die("Запрос клиента некорректно сформирован");
    }
    $lines = explode("\n", $_POST["data"]);
    if (count($lines) < 2) {
        die("В запросе недостаточное количество строк");
    }
    [$db_dl_time, $n_events] = explode("\t", $lines[0]);
    if (!isset($db_dl_time) || !isset($n_events)) {
        die("Некорректная первая строка запроса '" . $lines[0] . "'");
    }
    if ((int)$n_events != (count($lines) - 1)) {
        die("Некорректное количество данных из чипов '$n_events'");
    }
    ...
    try {
        ...
        for ($i = 1; $i <= $n_events; $i++) {
            ...
            if (count($event_data) != 10) {
                die("Некорректное количество данных в строке '" . $lines[$i] . "'");
            }
            $values = [$user_id, (int)$db_dl_time];
    ...
    $sql = $pdo->prepare(
        "INSERT INTO SportiduinoResults(team_num, sportiduino_teammask, levelpoint_order, teamlevelpoint_datetime) SELECT team_num, MIN(sportiduino_teammask), levelpoint_order, MAX(teamlevelpoint_datetime) FROM SportiduinoRecords WHERE sportiduinorecord_id > :max_id GROUP BY team_num, levelpoint_order"
    );
...
{
    if (!isset($request["X-Sportiduino-Time"])) {
        $from_time = "1970-01-01 00:00:00";
    } else {
        $from_time = $request["X-Sportiduino-Time"];
    }
    if (!preg_match("/[0-9 :-]+/", $from_time)) {
        die("Некорректное время начала данных");
    }
...
function ReceiveDatabase(PDO $pdo, $user_id)
{
    ...
    if ($file === false) {
        die("Ошибка создания файла '" . $fullname . "'");
    }
