...
<script language="JavaScript" type="text/javascript">
    ...
    // проверяем, что нет файла с таким именем
...
//	print('<input type="button" onClick="javascript: Cancel();" name="CancelButton" value="Отмена" tabindex="'.(++$TabIndex).'">'."\n");
    ...
    if (($viewmode !== "Add") && ($AllowEdit == 1)) {
        print('&nbsp; <input type="button" style="margin-left: 30px;" onClick="javascript: if (confirm(\'Вы уверены, что хотите удалить файл: ' . trim(
                $RaidFileName
            ) . '? \')) {HideFile();}" name="HideFileButton" value="Удалить файл" tabindex="' . (++$TabIndex) . '">' . "\n");
    }
...
while ($Row = mysqli_fetch_assoc($Result)) {
    ...
    print("<tr>\r\n");
    print('<td><a target="_blank" href="' . trim($MyStoreHttpLink) . trim(
            $Row['raidfile_name']
        ) . '">' . $Row['raidfile_name'] . '</a></td><td align="left">' . $Row['filetype_name'] . '</td><td>' . ($Row['raidfile_comment'] == '' ? '&nbsp;' : $Row['raidfile_comment']) . '</td>');
