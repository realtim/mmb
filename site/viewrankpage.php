...
class CTeamPlaces
{
    ...
    private function retrieveTeamPlaces(): void
    {
    ...
    private function retrieveUserDistance(): void
    {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $uid = $row['user_id'];
            if ($last !== $uid) {
                $this->userDistance[$uid] = [];
            }
            $last = $uid;

            $this->userDistance[$uid][$row['distance_id']] = [
...
$sql = " select MAX(r.raid_id) as maxraidid from Raids r";
