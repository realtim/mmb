...
if ($viewmode === 'Add') {
    ...
    } else // Пробуем создать команду первый раз
    {
        $RaidName = '';
        $RaidPeriod = '';
...
} else // ================ Редактируем/смотрим существующий ММБ =================
{
    if ($RaidId <= 0) {
        return;
    }
...
if ($viewmode !== 'Add') {
    ...
    while ($RowDistance = mysqli_fetch_assoc($DistanceResult)) {
        ...
        print('&nbsp; <input type="button" style="margin-left: 30px;" onClick="javascript: if (confirm(\'Вы уверены, что хотите удалить дистанцию.: ' . trim(
                $RowDistance['distance_name']
            ) . '? \')) {  DistanceDataForm' . $RowDistance['distance_id'] . '.action.value = \'HideDistance\';submit();}" name="HideFileButton' . $RowDistance['distance_id'] . '" value="Удалить" tabindex="' . (++$TabIndex) . '">' . "\n");
        print("</td></tr>\r\n");
        print("</table></form>\r\n");
    }
