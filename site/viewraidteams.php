...
<?php
	...
	//16,08.2015 функция не должна использоваться, так как теперь этот запрос в расчете результатов
	//Получить массив всех команд со списком невзятых точек для каждой
    function GetAllSkippedPoints($raidId)
    {
        $sql = "select t.team_id, t.distance_id,
						GROUP_CONCAT(lp.levelpoint_name ORDER BY lp.levelpoint_order, ' ') as notlevelpoint_name
				from  Teams t
						inner join  Distances d  on t.distance_id = d.distance_id
						join LevelPoints lp
...
*/
	...
	// Сортировка по месту требует более хитрого запроса
		...
		//Проверяем указана ли фильтрация по точке
	...
	while ($Row = mysql_fetch_assoc($Result))
	{
			...
			if ($OrderType == 'Place')   
			{
				...
				$skipped = isset($Row['team_skippedlevelpoint']) ? normalizeSkipped($Row['team_skippedlevelpoint']) : '&nbsp;';
