...
// выделяем в списках пропущенных КП интервалы. Закладываемся, что список точек отсортирован и без повторов.
    ...
    {
        $sql = "select t.team_id, GROUP_CONCAT(lp.levelpoint_name ORDER BY lp.levelpoint_order, ' ') as notlevelpoint_name
			from  Teams t
					inner join  Distances d
					on t.distance_id = d.distance_id
					join LevelPoints lp
...
//		print('<table width = "'.(($_REQUEST['LevelId'] > 0 and  $OrderType == 'Place') ? '1015' : '815').'" border = "0" cellpadding = "10" style = "font-size: 80%">'."\r\n");  
		...
		print("<div><small>Общее время: '" . ($t3-$t1) . "' подготовка: '" . ($prep - $t1 - $skpd) . "', запрос: '" . ($t2-$prep) . "' все пользователи: $allUsers, skipped: $skpd, выборка-отрисовка: " . ($t3-$t2 - $allUsers). '\'</small></div>');
?>
