...
?>
<form name="RaidTeamsForm" action="<?= $MyPHPScript ?>" method="post">
    ...
    if (($OrderType === 'Errors') && !$Administrator && !$Moderator) {
        $OrderType = '';
    }
    ...
    // если порядок не задан смотрим на соотношение времени публикации и текущего
    if (empty($OrderType)) {
    ...
    if ($CanViewResults || $Administrator || $Moderator)    // будет больше 1 пункта
    {
        ...
        //Сортировку по месту показываем только после окончания ММБ, если не стоит флаг "Не показывать результаты"
    ...
    ============================= точки ===============================
    */
    // 06/11/2015 Закомментировал условие фильтрации по результатам, чтобы ускорить запрос
    ...
    if (CRights::canShowPointsFilter($RaidId)) {
        ...
        if (!isset($_REQUEST['LevelPointId'])) {
            $_REQUEST['LevelPointId'] = "";
        }
    ...
    */
    ...
    if ($canShowMaps and $showMapImages) {
        ...
        while ($Row = mysqli_fetch_assoc($Result)) {
            ...
            print('<a style="margin-right: 15px;" href="' . trim($MyStoreHttpLink) . trim($ImageLink) . '" title="' . trim($ImageComment) . '" target = "_blank"><img src = "' . trim(
                    $MyStoreHttpLink
                ) . trim($tumbImg) . '"  alt = "' . trim($ImageComment) . '" height = "100"></a>' . "\r\n");
        }
        mysqli_free_result($Result);
        print('</div>' . "\r\n");
    }
    ...
    if (!empty($RaidCloseDt)) {
        ...
        if ($maxinvdt) {
            $message = "Заявка команд до $maxinvdt";
        } else {
            $message = "";
        }
        if ($lottery_count) {
            $message = "Заявка новых команд закрыта, добавление участников в существующие команды до $RaidRegisterEndDt";
        }
        if ($RaidStage == 2) {
            $message = "Добавление команд и участников уже невозможны, удаление закрывается за $RaidReadonlyHoursBeforeStart часов до старта";
        }
        if ($RaidStage == 3) {
            $message = "Любые изменения в командах уже невозможны, ждем вас на старте";
        }
        if (($RaidStage == 4) || ($RaidStage == 5)) {
            $message = "Результаты марш-броска обычно появляются через сутки после закрытия финиша";
        }
        if ($RaidStage > 5) {
            $message = "Об ошибках в результатах сообщайте в соответствующей теме в ЖЖ";
        }
        print($message);
    }
    print("</td></tr>\n");
    ...
    while ($Row = mysqli_fetch_assoc($Result)) {
        ...
        if ($Administrator && $outOfRange && in_array($Row['team_id'], $forgetful)) {
        ...
        if (!isset($TeamMembers[$Row['team_id']])) {
            ...
            foreach ($TeamMembers[$Row['team_id']] as $UserRow) {
                print('<div class= "input"><a href="?UserId=' . $UserRow['user_id'] . '&RaidId=' . $RaidId . '">' . CMmbUI::toHtml(
                        $UserRow['user_name']
                    ) . '</a> ' . $UserRow['user_birthyear'] . ' ' . CMmbUI::toHtml($UserRow['user_city']) . "\r\n");
        ...
        if ($OrderType === 'Place') {
            ...
            if (
                $Row['team_sresult'] === '00:00'
                || $Row['team_sresult'] == ''
                || $Row['team_outofrange'] == 1
                || $Row['team_error'] > 0
            ) {
                print('&nbsp;');
