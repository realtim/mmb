...
echo $MyPHPScript; ?>" method="post">
    ...
    <div style="margin-bottom: 2ex;">
        ...
        $allLevels = [CMmbLogger::Trace, CMmbLogger::Debug, CMmbLogger::Info, CMmbLogger::Warn, CMmbLogger::Error, CMmbLogger::Critical];
        $levels = [];
        $rawLevels = mmb_validate($_POST, 'levels', []);
        ...
        foreach ([100, 500, 1000, 5000] as $lim) {
        ...
        $sql = "select logs_id, logs_level, user_id, logs_operation, logs_message, logs_dt, logs_duration 
                from Logs 
        ...
        while ($Row = mysqli_fetch_assoc($Result)) {
            print("<tr><td>$cnt</td><td>{$Row['logs_id']}</td><td>" . $Row['logs_dt'] /*date("Y-m-d hh:mm:ss", $Row['logs_dt'])*/ . "</td><td>{$Row['logs_level']}</td><td>{$Row['user_id']}</td><td>{$Row['logs_operation']}</td><td>" . nl2br(
                    $Row['logs_message']
                ) . "</td><td>{$Row['logs_duration']}</td></tr>\n");
            $cnt++;
        }
