...
const PROGMEM byte chipSelect = 10; // SD-card pin

const PROGMEM byte Spk=8;  // BUZZER pin set
const PROGMEM byte GLed=9;  // GREEN LED pin set
const PROGMEM byte RLed=7;  // RED LED pin set

const PROGMEM byte TXPin = 4;
const PROGMEM byte RXPin = 3;
...
const PROGMEM char delimiter[]=", ";
const PROGMEM char pat_sign='*';
const PROGMEM char delimit_line[]=", Line#=";
...
void loop()
{
  ...
  dataString = cfgID + delimiter + cfgCP + delimiter + dataString + delimiter + GetDateTime();
  ...
  else  //Non config barcode
  {
    ...
    {
      if (cfgPattern[i]!=pat_sign && dataString[8+i]!=cfgPattern[i])  //String does not match the pattern
      {
  ...
  else
  {
    /*
        ...
        do {
          if (Serial.available())
          {
            dataString2+=char(Serial.read());
            strSize++;
          }
        } while (dataString2[strSize]!=0x0D && dataString2[strSize]!=0x0A);  //Getting the string from BT until CR+LF
    */
        Serial.print(F("Command received: "));
        Serial.println(dataString2);

        if (dataString2[0]=='S'&& dataString2[1]=='E'&& dataString2[2]=='T')  //SETTINGS
        {
        ...
        else if (dataString2[0]=='D' && dataString2[1]=='E' && dataString2[2]=='L' && dataString2[3]=='L' && dataString2[4]=='O' && dataString2[5]=='G')  //Remove datalog.txt and errors.txt
        {
        ...
        else if (dataString2[0]=='E'&& dataString2[1]=='N'&& dataString2[2]=='D')  //break BT operation mode
        {
