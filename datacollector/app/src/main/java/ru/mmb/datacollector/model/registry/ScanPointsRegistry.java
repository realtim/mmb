...
public class ScanPointsRegistry
{
	...
	public void refresh()
	{
		try
		{
			...
			scanPoints = DatacollectorDB.getConnectedInstance().loadScanPoints(CurrentRaid.getId());
			Collections.sort(scanPoints);

			levelPoints = DatacollectorDB.getConnectedInstance().loadLevelPoints(CurrentRaid.getId());
			...
			List<LevelPointDiscount> discounts =
			    DatacollectorDB.getConnectedInstance().loadLevelPointDiscounts(CurrentRaid.getId());
