...
public class ScanPointsRegistry {
	...
	public void refresh() {
		try {
			...
			scanPoints = DatabaseAdapter.getConnectedInstance().loadScanPoints(getCurrentRaidId());
			Collections.sort(scanPoints);

			levelPoints = DatabaseAdapter.getConnectedInstance().loadLevelPoints(getCurrentRaidId());
			...
			List<LevelPointDiscount> discounts = DatabaseAdapter.getConnectedInstance().loadLevelPointDiscounts(
					getCurrentRaidId());
