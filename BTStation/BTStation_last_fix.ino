...
void getTeamRecord()
{
	...
	//13-14: счетчик сохраненных страниц
	bool flag = true;
...
byte writeDumpToFlash(uint16_t recordNum, uint32_t checkTime)
{
	...
	while (block < TAG_MAX_PAGE - 3)
	{
		...
		for (byte i = 0; i < 4; i++)
		{
			...
			{
				flag = true;
				flag &= SPIflash.writeByte(teamFlashAddress + (uint32_t)block * (uint32_t)4 + (uint32_t)i * (uint32_t)4 + (uint32_t)0, ntag_page[0 + i * 4]);
				flag &= SPIflash.writeByte(teamFlashAddress + (uint32_t)block * (uint32_t)4 + (uint32_t)i * (uint32_t)4 + (uint32_t)1, ntag_page[1 + i * 4]);
				flag &= SPIflash.writeByte(teamFlashAddress + (uint32_t)block * (uint32_t)4 + (uint32_t)i * (uint32_t)4 + (uint32_t)2, ntag_page[2 + i * 4]);
				flag &= SPIflash.writeByte(teamFlashAddress + (uint32_t)block * (uint32_t)4 + (uint32_t)i * (uint32_t)4 + (uint32_t)3, ntag_page[3 + i * 4]);
				if (flag) return false;
				checkCount++;
#ifdef DEBUG
	...
	if (checkCount > 0)
	{
		SPIflash.writeByte((uint32_t)((uint32_t)recordNum * (uint32_t)LOG_RECORD_LENGTH + (uint32_t)12), (checkCount & 0x0000FF00) >> 8);
		SPIflash.writeByte((uint32_t)((uint32_t)recordNum * (uint32_t)LOG_RECORD_LENGTH + (uint32_t)13), checkCount & 0x000000FF);
	}
	return true;
}
